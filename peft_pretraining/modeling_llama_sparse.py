...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.mup_input_alpha = config.mup_input_alpha
        self.mup_output_alpha = config.mup_output_alpha
    ...
    def forward(
        ...
        attn_output = torch.nn.functional.scaled_dot_product_attention(
            query_states, key_states, value_states, dropout_p=0.0, is_causal=True, scale=attention_scale
        )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if self.mup_enabled:
            ### Begin muP code ###
            hidden_states *= self.mup_input_alpha
            ### End muP code ###
...
class LlamaForCausalLM(LlamaPreTrainedModel):
        ...
        if self.mup_enabled:
            for pn, p in self.named_parameters():
                if (pn.endswith('proj.weight')):
                    scaling = math.sqrt(self.mup_width_multiplier)
                    std = config.initializer_range / scaling
                    torch.nn.init.normal_(p, mean=0.0, std=std)
                    print("density aware init for layer: {}--{}".format(pn, config.density))
    ...
    def forward(
        ...
        if self.mup_enabled:
            ...
            hidden_states *= self.mup_output_alpha / self.mup_width_multiplier
            ### End muP code ###
